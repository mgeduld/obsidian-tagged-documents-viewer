...
export default class PaginationControls {
	...
	public changeCurrentPage(newPage: number) {
		...
		this.container
			.querySelector(`[data-page-${oldPage}]`)
			?.removeClasses(["current-page-button"]);
		this.container
			.querySelector(`[data-page-${newPage}]`)
			?.addClasses(["current-page-button"]);
	...
	public renderPaginationControls(totalDocs: number) {
		...
		const previoiusButton = this.container.createEl("button", {
