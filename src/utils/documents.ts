...
const getDocument = async (file: TFile, reader: Function): Promise<string> => {
    let fileText = await reader(file)
    if (fileText.length > MAX_DOC_LENGTH) {
        fileText = fileText.substring(0, MAX_DOC_LENGTH) + '... *[File truncated]*'
    }
    return fileText;
}
...
export const getDocuments = async (
    ...
    const files = filesWithTags.slice(startIndex, startIndex + MAX_DOCS_PER_PAGE);
